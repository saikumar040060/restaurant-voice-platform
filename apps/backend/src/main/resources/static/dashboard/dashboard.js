(() => {
  ...
  const writeReviewDecision = async (item, decision, correction, rationale, known) => {
    const response = await fetch("/api/v1/restaurant/menu-review-draft/decisions", {
      method: "POST", headers: { Authorization: `Bearer ${token()}`, "Content-Type": "application/json" },
      body: JSON.stringify({ itemIndex: item.source_index, decision, correction, rationale, expectedVersion: known?.version || 0 })
    });
    if (response.status === 401) { signedOut(); throw new Error("Your session expired. Please sign in again."); }
  ...
  const review = async () => {
    const [data, saved, summary, completionView] = await Promise.all([request("/api/v1/restaurant/menu-review-draft"), request("/api/v1/restaurant/menu-review-draft/decisions"), request("/api/v1/restaurant/menu-review-draft/summary"), request("/api/v1/restaurant/menu-review-draft/completion")]);
    const completion = completionView.completion;
    const decisions = Object.fromEntries(saved.map(decision => [decision.itemIndex, decision])); clear();
    const flagged = data.items.filter(item => item.review_flags.length);
    const sections = [["Flagged issues", flagged], ["Remaining draft entries", data.items.filter(item => !item.review_flags.length)]];
    const note = document.createElement("p"); note.textContent = `Owner review only. ${summary.approved} approved, ${summary.corrected} corrected, ${summary.rejected} rejected, and ${summary.pending} pending. Decisions are tenant-scoped, versioned, audited, and remain unpublished. Import and publication are disabled.`; content.append(note);
    const decide = async (item, decision, correction, rationale) => { try { await writeReviewDecision(item, decision, correction, rationale, decisions[item.source_index]); await review(); } catch (error) { setStatus(error.message); } };
    sections.forEach(([section, sectionItems]) => { const sectionHeading = document.createElement("h2"); sectionHeading.textContent = section; content.append(sectionHeading); Object.entries(Object.groupBy(sectionItems, item => item.category)).forEach(([category, items]) => { const heading = document.createElement("h3"); heading.textContent = category; content.append(heading); const rows = items.map(item => [item.name, item.listed_price, item.review_flags.join(", ") || "none", decisions[item.source_index]?.decision || "PENDING"]); content.append(table(["Item", "Price", "Flags", "Unpublished decision"], rows)); items.forEach(item => { const controls = document.createElement("p"); [["approve", "APPROVED"], ["reject", "REJECTED"]].forEach(([label, decision]) => { const button = document.createElement("button"); button.textContent = `${label} ${item.name}`; button.addEventListener("click", () => { const rationale = decision === "REJECTED" ? prompt("Why should this entry be rejected? (unpublished owner review)") : null; if (decision !== "REJECTED" || rationale) decide(item, decision, null, rationale); }); controls.append(button); }); const correct = document.createElement("button"); correct.textContent = `correct ${item.name}`; correct.addEventListener("click", () => { const correction = prompt("Owner correction (unpublished review only):"); if (correction) decide(item, "CORRECTED", correction, null); }); controls.append(correct); content.append(controls); }); }); });
    const undecidedFlagged = flagged.filter(item => !decisions[item.source_index]);
    if (undecidedFlagged.length) { const bulk = document.createElement("button"); bulk.textContent = `Approve ${undecidedFlagged.length} undecided flagged entries (unpublished)`; bulk.addEventListener("click", async () => { try { const response = await fetch("/api/v1/restaurant/menu-review-draft/decisions/bulk", { method: "POST", headers: { Authorization: `Bearer ${token()}`, "Content-Type": "application/json" }, body: JSON.stringify({ decisions: undecidedFlagged.map(item => ({ itemIndex: item.source_index, decision: "APPROVED", correction: null, rationale: null, expectedVersion: 0 })) }) }); if (response.status === 401) { signedOut(); throw new Error("Your session expired. Please sign in again."); } if (response.status === 409) throw new Error("A review item changed. Nothing was saved; reload and try again."); if (!response.ok) throw new Error("Bulk decision could not be saved."); await review(); } catch (error) { setStatus(error.message); } }); content.prepend(bulk); }
    if (completion) { const complete = document.createElement("p"); complete.textContent = `Review completed and locked as UNPUBLISHED at ${completion.completedAt}.`; content.prepend(complete); }
    else if (summary.pending === 0) { const complete = document.createElement("button"); complete.textContent = "Complete and lock review (still unpublished)"; complete.addEventListener("click", async () => { try { const response = await fetch("/api/v1/restaurant/menu-review-draft/completion", { method: "POST", headers: { Authorization: `Bearer ${token()}` } }); if (!response.ok) throw new Error("Review completion could not be recorded."); await review(); } catch (error) { setStatus(error.message); } }); content.prepend(complete); }
  };
