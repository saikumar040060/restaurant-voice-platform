(() => {
  ...
  const menu = async () => { const data = await request("/api/v1/restaurant/menu"); clear(); content.append(table(["Item", "Price", "Required choices"], data.items.map(item => [item.name, `$${(item.priceMinor / 100).toFixed(2)}`, [...item.requiredModifierGroups].join(", ")]))); };
