(() => {
  ...
  const reviewKey = "harborvoice.menu-review.decisions";
  const decisions = () => JSON.parse(sessionStorage.getItem(reviewKey) || "{}");
  const saveDecision = (id, decision) => { const value = decisions(); value[id] = decision; sessionStorage.setItem(reviewKey, JSON.stringify(value)); };
  const review = async () => { const data = await request("/api/v1/restaurant/menu-review-draft"); clear(); const flagged = data.items.filter(item => item.review_flags.length); const byCategory = Object.groupBy(flagged, item => item.category); const note = document.createElement("p"); note.textContent = "Owner review only. Decisions are stored only for this browser session and cannot import or publish this draft."; content.append(note); Object.entries(byCategory).forEach(([category, items]) => { const heading = document.createElement("h2"); heading.textContent = category; content.append(heading); const rows = items.map(item => [item.name, item.listed_price, item.review_flags.join(", "), decisions()[item.source_index]?.status || "pending"]); content.append(table(["Item", "Price", "Flags", "Session decision"], rows)); items.forEach(item => { const controls = document.createElement("p"); ["approve", "reject"].forEach(status => { const button = document.createElement("button"); button.textContent = `${status} ${item.name}`; button.addEventListener("click", () => { saveDecision(item.source_index, { status }); review(); }); controls.append(button); }); const correct = document.createElement("button"); correct.textContent = `correct ${item.name}`; correct.addEventListener("click", () => { const correction = prompt("Owner correction (review only):"); if (correction) { saveDecision(item.source_index, { status: "correct", correction }); review(); } }); controls.append(correct); content.append(controls); }); }); const bulk = document.createElement("button"); bulk.textContent = "Approve all currently flagged entries for this session"; bulk.addEventListener("click", () => { flagged.forEach(item => saveDecision(item.source_index, { status: "approve" })); review(); }); content.prepend(bulk); };
