(() => {
  ...
  const review = async () => {
    ...
    const bulk = document.createElement("button"); bulk.textContent = "Approve all currently flagged entries (unpublished)"; bulk.addEventListener("click", async () => { try { for (const item of flagged) await writeReviewDecision(item, "APPROVED", null, decisions[item.source_index]); await review(); } catch (error) { setStatus(error.message); } }); content.prepend(bulk);
  };
