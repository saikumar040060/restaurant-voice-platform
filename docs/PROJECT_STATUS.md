...
Immutable-draft source-index boundary: menu-review writes now accept only indexes 1 through 256, matching the fixed current review source. Invalid or invisible indexes fail before any decision or audit event is written. The owner ran the disposable PostgreSQL harness on 2026-09-08: 172 tests passed with zero failures/errors. Import and publication remain disabled.

Owner dashboard completeness: the menu-review view now groups every one of the 256 non-executable draft entries by category, with flagged issues shown before the remaining entries. All controls still create only tenant-scoped, versioned, audited `UNPUBLISHED` decisions; no import, publication, ordering, payment, Railway, Twilio, or provider action was added. The owner ran `python3 scripts/test_backend.py` on 2026-09-08: 172 tests passed with zero failures/errors.

Protected bulk review: bulk approval is now limited to undecided flagged entries. Existing owner corrections and rejections cannot be overwritten through the bulk endpoint; the backend rejects such requests before writing decisions or audit events. The owner reran `python3 scripts/test_backend.py` on 2026-09-08: 172 tests passed with zero failures/errors. All menu data remains unpublished.

Rejection rationale: rejecting a menu-review entry now requires a bounded owner explanation, stored with the tenant-scoped unpublished decision. The database enforces the rejection/rationale relationship, and the dashboard collects the explanation before submission. The owner ran `python3 scripts/test_backend.py` on 2026-09-08: 172 tests passed with zero failures/errors. No menu import, publication, provider, order, or payment behavior changed.

Synthetic menu-review completion gate: all 256 current-revision entries must have a decision before the owner-only completion endpoint succeeds. Completion stores an immutable SHA-256 decision-set hash, remains `UNPUBLISHED`, writes one transactional audit event, is idempotent, and prevents later decision mutation through both application and PostgreSQL controls. The dashboard exposes the lock only when pending count reaches zero. The owner ran `python3 scripts/test_backend.py` on 2026-09-08: 173 tests passed with zero failures/errors. No import, publication, Railway, Twilio, call, order, or payment behavior changed.

Final test-only menu-review report: the owner-only report endpoint renders the locked 256-entry decision set by source category and includes each listed price, decision, correction or rejection rationale, immutable draft revision, decision-set SHA-256 hash, completion time, and completion audit-event ID. It is labeled `UNPUBLISHED — TEST DATA` and fails closed until the review is complete. The owner ran `python3 scripts/test_backend.py` on 2026-09-08: 173 tests passed with zero failures/errors, followed by successful package/repackage and secure bootstrap checks. No live service or menu publication changed.

Test-only import preview: an owner-only read endpoint transforms the completed locked review into a non-executable canonical preview. Approved and corrected entries are included, rejected entries are excluded with their rationale, deterministic preview SKUs are assigned, fixed prices are converted to minor units, and unclear prices, free-text corrections, or unresolved source flags remain explicit import blockers. The response is labeled `UNPUBLISHED — TEST-ONLY IMPORT PREVIEW` and has no write, import, publication, POS, order, or payment capability. The owner ran `python3 scripts/test_backend.py` on 2026-09-08: 173 tests passed with zero failures/errors and secure packaged-bootstrap checks.

Indian-menu synthetic conversation evaluation: five provider-free transcript scenarios now verify an exact grounded price answer from the locked draft, clarification for a duplicated dish name, refusal to invent an unknown dish, human transfer for unverified allergy/dietary claims, and rejection of ordering/correction requests while the menu remains an unpublished preview. The focused suite passed 5/5, and the owner then ran `python3 scripts/test_backend.py` on 2026-09-08: 178 tests passed with zero failures/errors plus successful package/repackage and secure bootstrap checks. No model, audio provider, live call, order, payment, import, or publication was used.

M7 test-call media bridge: the signed Twilio webhook now places its short-lived, one-use admission token in a TwiML custom parameter, and the disabled-by-default Media Streams handler consumes it only from Twilio's start event. An admitted call opens the provider-neutral realtime session, configures PCM mu-law audio in both directions with server VAD and bounded output, forwards bounded synthetic frames, and returns only provider audio to the same stream. The sandbox prompt uses the bundled unpublished Indian-menu names, categories, and listed prices while prohibiting real orders, payments, recordings, and allergy claims. Invalid, oversized, out-of-order, and replayed events fail closed; session cleanup releases the existing duration/concurrency/spend lease. The owner ran `python3 scripts/test_backend.py` on 2026-09-08: 183 tests passed with zero failures/errors, successful package/repackage, and secure bootstrap checks. No live service was changed and no real call occurred during this verification.

Live sandbox menu grounding fix: the Realtime session prompt now explicitly tells the assistant that it has the complete 256-entry test menu and includes each supplied description with its source index, category, item name, and listed price. It must answer grounded menu questions, clarify duplicate or unclear entries, retain a fake cart during the call, and read back that nothing will be submitted. Menu publication, provider tools, recordings, allergy claims, real ordering, and payments remain prohibited.

Live sandbox continuity fix: the bounded Realtime output queue now retains an ordinary 128-chunk provider audio burst instead of discarding speech after 32 queued chunks. The queue remains capped at 512 chunks. Focused regression coverage verifies the burst drains completely; no recording, ordering, payment, or provider tool was added.

Immediate Realtime playback: provider audio now flows through a provider-neutral output callback and is sent to the admitted Twilio stream as soon as OpenAI emits it, without waiting for another inbound phone frame. Socket writes are serialized and fail closed. The live sandbox duration is three minutes and individual responses are capped at 1,024 tokens; concurrency and total-spend controls remain unchanged.

Natural-response tuning: the call opens with a short ordinary greeting, server VAD responds after 350 ms of silence, and the 256-entry menu prompt uses bounded 80-character source summaries instead of full descriptions. The assistant is instructed to answer in one or two conversational sentences, avoid repeating technical/test language during normal menu questions, and answer honestly if directly asked whether it is automated. Fake-order read-back still states that nothing is submitted.

Multi-turn barge-in fix: the provider-neutral Realtime boundary now signals caller-speech interruptions to the phone adapter. Twilio receives an immediate `clear` event, removing buffered speech before the next answer is streamed. Synthetic coverage verifies first answer, caller interruption/new question, buffer clear, and second answer in one admitted call.

Live conversational retrieval fix: the Realtime prompt no longer embeds the 256-entry Indian menu on every call. A provider-neutral, tenant-scoped, read-only `restaurant_menu_lookup` gateway now loads the immutable unpublished draft once, returns at most eight grounded matches (or a short category list), preserves review flags and unverified-allergen state, and denies cross-tenant and all non-menu tools. Realtime function-call results are returned to the same call before generation resumes; no order, payment, customer-data, provider, or administrative tool is exposed. The live voice is now `marin`, while 350 ms server VAD and existing barge-in clearing remain in place. Focused media/session/retrieval/admission tests passed: 21 tests, zero failures/errors; foundation validation and Java 21 packaging passed. The required Docker harness was attempted, but this Codex execution environment was still denied access to `/Users/saikumar/.docker/run/docker.sock`; a local disposable PostgreSQL fallback was also blocked by sandbox shared-memory policy. The previous owner-run full suite passed 183 tests before this isolated provider-layer change; full post-change Docker verification remains unclaimed.

Test-call pickup intake: when a caller starts a fake pickup order, the realtime assistant now collects the pickup name before the first item and separately asks whether the calling number is the best callback number. It asks for another number only after a negative answer, never reads full digits aloud, never treats caller ID as identity proof, retains the values only in call memory, and includes only the pickup name in the fake-order read-back. Real order creation and persistence remain disabled. Focused retrieval/media/session/prompt coverage passed: 19 tests, zero failures/errors; foundation validation passed. The approved Railway sandbox model was also changed from `gpt-realtime-2.1-mini`, which did not meet the owner's live conversational quality threshold, to `gpt-realtime-2.1`; the stronger-model Railway deployment completed successfully under the existing sandbox account and call limits.
